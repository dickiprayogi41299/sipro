 ...
 bs4DashSidebar(
  ...
  "))
                ...
                fluidRow(
                  ...
                  div(class = "col-12 col-sm-6 col-md-6 col-lg-4",
                      infoBox(
                        ...
                        value = tags$a(
                          href = "https://1drv.ms/f/c/9e7e9ee182eb06e5/EmCtERpJiqtBj4dKBRhmdRUB8GEAHBRXYokXrX0moxxVKQ?e=vFlb0u",
                          target = "_blank",
                          icon("database"),
                          " Akses Data Historis"
                        ),
                    ...
                    tabPanel(
                      ...
                      fluidRow(
                        ...
                        column(
                          ...
                          tags$div(
                            style = "padding: 10px;",
                            tags$h4("Bahan Paparan Ekspor dan Impor"),
                            tags$p("Bahan paparan ekspor dan impor bulanan secara periodik dapat diakses pada link berikut."),
                            tags$a(
                              href = "https://drive.google.com/drive/folders/1RIJYqI2st8o7I-gntG2713Ask5iEtI0y?usp=sharing",
                              target = "_blank",
                              class = "btn btn-primary",
                              icon("external-link-alt"),
                              HTML("&nbsp;Lihat Bahan Paparan")
                            )
                          )
                        )
                      )
                ...
                fluidRow(
                  column(
                    ...
                    box(
                      ...
                      numericInput("jumlah_file", "Berapa file yang ingin digabung?", value = 1, min = 1),
                    ...
                    box(
                      title = tagList(icon("table"), HTML("&nbsp;&nbsp;Clean Data")),
                      status = "primary",
                      solidHeader = TRUE,
                      width = NULL,  # full lebar column
                      div(
                        style = "display: flex; flex-wrap: wrap; gap: 10px; justify-content: flex-start;",
                        downloadButton("downloadDataClean", "Download", style = "flex: 1 1 200px;")
                      ),
                      tags$br(), tags$br(),
                ...
                box(
                  ...
                  tabBox(
                    ...
                    tabPanel(
                      ...
                      div(
                        style = "display: flex; flex-wrap: wrap; gap: 10px; justify-content: flex-start;",
                        downloadButton("downloadData", "Download", style = "flex: 1 1 200px;"),
                ...
                box(
                  ...
                  tabBox(
                    ...
                    tabPanel(
                      ...
                      div(
                        style = "display: flex; flex-wrap: wrap; gap: 10px; justify-content: flex-start;",
                        downloadButton("downloadDataImpor", "Download", style = "flex: 1 1 200px;"),
                ...
                box(
                  ...
                  tabBox(
                    ...
                    tabPanel(
                      ...
                      div(
                        style = "display: flex; flex-wrap: wrap; gap: 10px; justify-content: flex-start;",
                        downloadButton("downloadDataNeraca", "Download", style = "flex: 1 1 200px;"),
    ...
    "))
            ...
            tags$a(
              href = "https://wa.me/6285763542884?text=Halo%2C%20saya%20ingin%20bertanya%20terkait%20SIPRO",
              target = "_blank",
              class = "wa-float",
    ...
    tags$footer(class = "main-footer",
                ...
                tags$strong("BADAN PUSAT STATISTIK", style = "color: #3c8dbc;")
    )
  )
)
...
server <- function(input, output, session) {
  ...
  observeEvent(input$file_input, {
    ...
    if (!all(c("TAHUN", "BULAN") %in% names(df))) {
      showNotification("Kolom 'TAHUN' atau 'BULAN' tidak ditemukan di data.", type = "error")
      return()
    }
   ...
   tabel12<<-tabel8_1
    ...
    output$downloadData <- downloadHandler(
      ...
      content = function(file) {
        ...
        # Teks catatan
        footer_text <- paste("Data ini diolah menggunakan SIPRO pada", timestamp)
  ...
  observeEvent(input$file_input_neraca_ekspor, {
    ...
    if (!validasi_kolom_tahun_bulan(df)) {
      showNotification("Data ekspor tidak memiliki kolom 'TAHUN' dan/atau 'BULAN'.", type = "error")
      return()
    }
  ...
  observeEvent(input$file_input_neraca_impor, {
    ...
    if (!validasi_kolom_tahun_bulan(df)) {
      showNotification("Data impor tidak memiliki kolom 'TAHUN' dan/atau 'BULAN'.", type = "error")
      return()
    }
...
observeEvent(input$confirm_tahun_button, {
  ...
  observeEvent(input$clean_button, {
    ...
    gabungan <- dplyr::bind_rows(data_storage$data_list)
    gabungan <- dplyr::arrange(gabungan, desc(TAHUN), desc(BULAN))
    
    gabungan$BULAN <- as.numeric(gabungan$BULAN)
    gabungan$TAHUN <- as.numeric(gabungan$TAHUN)
    
    gabungan$PROPINSI <- trimws(as.character(gabungan$PROPINSI), which = "both")
    gabungan$PELABUHAN <- trimws(as.character(gabungan$PELABUHAN), which = "both")
    
    gabungan$HS <- trimws(as.character(gabungan$HS), which = "both")
    gabungan$HS <- ifelse(nchar(gabungan$HS) == 7, paste0("0", gabungan$HS), gabungan$HS)
    
    gabungan$NEGARA <- trimws(as.character(gabungan$NEGARA), which = "both")
    
    gabungan$BERAT <- as.numeric(gabungan$BERAT)
    gabungan$NILAI <- as.numeric(gabungan$NILAI)
    
    gabungan$HSDUA <- trimws(as.character(gabungan$HSDUA), which = "both")
    gabungan$HSDUA <- ifelse(nchar(gabungan$HSDUA) == 1, paste0("0", gabungan$HSDUA), gabungan$HSDUA)
    
    gabungan$KABKOT <- trimws(as.character(gabungan$KABKOT), which = "both")
    gabungan$PELABUHAN1 <- trimws(as.character(gabungan$PELABUHAN1), which = "both")
    gabungan$NEGARA2 <- trimws(as.character(gabungan$NEGARA2), which = "both")
    gabungan$HS2 <- trimws(as.character(gabungan$HS2), which = "both")
    gabungan$JENIS <- trimws(as.character(gabungan$JENIS), which = "both")
    gabungan$SEKTOR <- trimws(as.character(gabungan$SEKTOR), which = "both")
    gabungan$KOMODITI <- trimws(as.character(gabungan$KOMODITI), which = "both")
    
    data_bersih(gabungan)
    
    shinyjs::disable("downloadDataClean")  # langsung disable tombol
    
    output$downloadDataClean <- downloadHandler(
      filename = function() {
        paste0("data_bersih_", Sys.Date(), ".sav")
      },
      content = function(file) {
        req(data_bersih())
    ...
    observe({
      if (!is.null(data_bersih()) && nrow(data_bersih()) > 0) {
        shinyjs::enable("downloadDataClean")  # aktifkan kalau data siap
      } else {
        shinyjs::disable("downloadDataClean") # disable kalau data gak ada
      }
    })
